...
- 更新文档时进程 PID：`27536`
- 当前运行输出目录：`data\20260720T095507Z`
- 更新文档时 BTC、ETH 均无持仓。
- 更新文档时 BTC 市场为 `btc-updown-5m-1784541300`，ETH 市场为 `eth-updown-5m-1784541300`，两者起止时间完全一致。
- 实际联网验证已确认：BTC/ETH 当前市场、各自 UP/DOWN token、两套 CLOB 盘口、`BTCUSDT`/`ETHUSDT` 和 `BTC/USD`/`ETH/USD` 实时 tick 均正常。
- 配对模块当前已由用户开启；每腿 10 USD、最低价差 10 美分、开盘后 `[20, 280)` 秒、每场上限 2、严格交替开启。更新文档时账本有 1 单待官方结算。
- 最近完整测试：`167 passed`
...
每腿分别按相同 USD 金额扫多档卖盘，买入手续费额外计入成本，份数允许不同。只有四个盘口都可信、未过期、深度足够且达到最小订单量时才允许入场；配对入场不依赖单币阈值核验。价差为：

```text
价差（美分） = 100 × [1 - BTC均价 - ETH均价 - BTC手续费/份数 - ETH手续费/份数]
```

相同四腿卖盘组合快照最多开一组。严格交替开启时，首单选达标组合中价差较高者，此后只等待相反方向；关闭时每次选当前达标组合中价差较高者。每个对齐场次都会重新开始交替。模块启用后，BTC/ETH 单币引擎停止新入场，但已经存在的单币仓位仍按原规则退出和结算；配对条件不足时不会恢复单币入场。
